describe(require('../../__fixtures/utils/test_helper').create().testName(__filename, 3), function () {
  ...
  before('creates 20 users with substring usernames', function (done) {
    ...
    async.times(20, function(time, timeCB){
  ...
  it('checks to see that only the correct user is fetched', function (done) {
    ...
    async.times(20, function(time, timeCB){
      ...
      serviceInstance.services.security.users.getUser(username, {
        overwrite: false
      }, function (e, user) {
        if (e) return timeCB(e);
        if (user.username != username) return timeCB(new Error('usernames do not match:' + user.username + ' ' + username));
        timeCB();
      });
  ...
  it('checks to see that only the correct user is fetched, negative test', function (done) {
    ...
    serviceInstance.services.security.users.getUser = Promisify(function (userName, options, callback) {
      ...
      _this.dataService.get(searchPath, {
        ...
        if (!user) return callback(new Error('group links exist for non-existing user: ' + userName));
