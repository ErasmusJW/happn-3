(function () { // begin enclosed
  ...
  HappnClient.create = Promisify(function (connection, options, callback) {
    ...
    var clientOptions = __prepareOptions(connection, options);
      ...
      function (attempt, next) {
        ...
        login(function (e) {
          ...
          if (e) {
            ...
            if (e.code && e.code == 403) return next(e); //access was denied
  ...
  var lastNMessagesPrinted = false;

  var lastNSize = 200;

  var lastNWriteAttempts = [];
  var lastNWriteFailures = [];
  var lastNWriteSuccesses = [];

  var lastWriteAttemptPayload = null;
  var lastWriteSuccessPayload = null;
  var lastWriteFailedPayload = null;

  HappnClient.prototype.__printLastNMessages = function(){

    if (lastNMessagesPrinted) return;

    lastNMessagesPrinted = true;

    console.log('--------------------------ATTEMPTS----------------------------');

    lastNWriteAttempts.forEach(function(attempt){
      console.log(attempt.timestamp, '    ', attempt.action, attempt.path);
    });

    if (lastWriteAttemptPayload) {
      console.log('--------------------------ATTEMPTS LAST PAYLOAD----------------------------');
      console.log(JSON.stringify(lastWriteAttemptPayload, null, 2));
    }

    console.log('--------------------------SUCCESSES----------------------------');

    lastNWriteSuccesses.forEach(function(attempt){
      console.log(attempt.timestamp, '    ', attempt.action, attempt.path);
    });

    if (lastWriteSuccessPayload) {
      console.log('--------------------------SUCCESSES LAST PAYLOAD----------------------------');
      console.log(JSON.stringify(lastWriteSuccessPayload, null, 2));
    }

    console.log('--------------------------FAILURES----------------------------');

    lastNWriteFailures.forEach(function(attempt){
      console.log(attempt.timestamp, '    ', attempt.action, attempt.path);
    });

    if (lastWriteFailedPayload) {
      console.log('--------------------------FAILURES LAST PAYLOAD----------------------------');
      console.log(JSON.stringify(lastWriteFailedPayload, null, 2));
    }
  };

  HappnClient.prototype.__pushLastNArray = function(arr, message){

    if (arr.length == lastNSize) arr.shift();
    arr.push({timestamp:new Date(), path:message.path, action:message.action});
  };

  HappnClient.prototype.__pushLastNSuccesses = function(message){

    this.__pushLastNArray(lastNWriteSuccesses, message);
    lastWriteSuccessPayload = message.data;
  };

  HappnClient.prototype.__pushLastNAttempts = function(message){
    this.__pushLastNArray(lastNWriteAttempts, message);
    lastWriteAttemptPayload = message.data;
  };

  HappnClient.prototype.__pushLastNFailures = function(message){
    this.__pushLastNArray(lastNWriteFailures, message);
    lastWriteFailedPayload = message.data;
  };
  ...
  HappnClient.prototype.__requestCallback = function (message, callback, options, eventId, path, action) {
    ...
    callbackHandler.handleResponse = function (e, response) {
      ...
      _this.__pushLastNSuccesses(this.message);
    ...
    callbackHandler.timedout = setTimeout(function () {
      ...
      if (this.message.path) {
        ...
        if (this.message.path.indexOf('deviceStatus') == -1 && !browser) {

          console.log("Process Exit started");

          _this.__pushLastNFailures(this.message);

          _this.__printLastNMessages();
  ...
  HappnClient.prototype.__performDataRequest = function (path, action, data, options, callback) {
    ...
    if (this.state != STATE.ACTIVE) {
      console.log("%s - Client state not active, state=%d", (new Date()).toString(), this.state);
    ...
    this.__pushLastNAttempts(message);
  ...
  HappnClient.prototype.handle_error = function (err, severity) {
    ...
    console.log("%s - happn client handle_error", (new Date()).toString(), err, severity);
