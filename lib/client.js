(function() { // begin enclosed
  ...
  HappnClient.prototype.__getVariableDepthPermutationPaths = function(path, depth, trailing){
    ...
    for (var i = 0; i <= depth; i++){
      ...
      if (!trailing) permutations.push(path.replace('/**/', '/' + permutationArray.join('/') + '/'));
      else permutations.push(path.replace('/**', '/' + permutationArray.join('/')));
    }
  ...
  HappnClient.prototype.__onVariableDepth = Promisify(function(path, parameters, handler, callback) {
    ...
    var trailing = path.substring(path.length - 3) == '/**';

    var enclosedMatch = path.match(/\/\*\*\//g);

    var enclosed = enclosedMatch?enclosedMatch.length:0;

    var adjacent = path.indexOf('/**/**/') > -1;

    if (enclosed == 0 && !trailing) return callback(new Error('variable depth segments must either be trailing, or be enclosed by segment delimiters /, ie: /my/test/** or /my/**/test is ok, this is not ok: /my/tes**/1'));

    if (enclosed > 1 || enclosed == 1 && trailing || adjacent) return callback(new Error('variable depth subscription paths can only have one variable depth segment, ie: this is not ok /my/test/**/path/**'));

    if (!parameters.depth) parameters.depth = 5;

    var variableDepthPermutationPaths = _this.__getVariableDepthPermutationPaths(path, parameters.depth, trailing);

    var basePath = variableDepthPermutationPaths[0]; //the primary path we use to get a base handle

    var allInitialValues = undefined;
  ...
  HappnClient.prototype.on = Promisify(function(path, parameters, handler, callback) {
    ...
    //variable depth call
    if (path.indexOf('**') >= 0) return _this.__onVariableDepth(path, parameters, handler, callback);
