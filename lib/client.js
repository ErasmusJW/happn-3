(function () { // begin enclosed
  ...
  HappnClient.prototype.__encryptPayload = function (message) {
    ...
    var iv = this.utils.computeiv(this.session.secret);
    payload = crypto.symmetricEncryptObjectiv(
      message,
      self.session.secret,
      iv
    );
  ...
  HappnClient.prototype.__decryptPayload = function (message) {
    ...
    var iv = self.utils.computeiv(self.session.secret);
  ...
  HappnClient.prototype.__reattachListeners = function (callback) {
    ...
    _this.utils.async(Object.keys(_this.state.events), function(eventPath, index, nextEvent) {
      ...
      _this.utils.async(listeners, function(listener, index, nextListener) {
        ...
        _this._remoteOn(eventPath, parameters, function(e, response) {
          ...
          if (e) {
            if ([403, 401].indexOf(e.code) > -1) {//permissions may have changed regarding this path
              delete _this.state.events[eventPath];
              return nextListener();
            }
            return nextListener(new Error('failed re-establishing listener to path: ' + eventPath, e));
          }
  ...
  HappnClient.prototype.__confirmRemoteOn = function(path, parameters, listener, callback){
    ...
    _this._remoteOn(path, parameters, function(e, response) {
      ...
      if (listener.initialEmit) {//emit data as events immediately
        response.forEach(function(message) { listener.handler(message);});
        _this.__updateListenerRef(listener, response._meta.referenceId);
      } else if (listener.initialCallback) {//emit the data in the callback
        _this.__updateListenerRef(listener, response._meta.referenceId);
      } else {
        _this.__updateListenerRef(listener, response.id);
      }
  ...
  HappnClient.prototype.on = Promisify(function(path, parameters, handler, callback) {
    ...
    var variableDepth = typeof path == 'string' && (path == '**' || path.substring(path.length - 3) == '/**');
    ...
    if (!(this.state.refCount[listener.eventKey] == 1 || listener.initialCallback || listener.initialEmit))
  ...
  HappnClient.prototype.disconnectCleanup = function (options){
    this.clearTimeouts();
  ...
  HappnClient.prototype.disconnect = Promisify(function (options, callback) {
    ...
    try {
      ...
      this.clearTimeouts();
      this.disconnectCleanup(options);
