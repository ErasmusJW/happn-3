(function() { // begin enclosed
  ...
  HappnClient.prototype.__initializeState = function() {
    ...
    _this.state.variableDepthSubscriptions = {};
  };
  ...
  HappnClient.prototype.__getVariableDepthPermutationPaths = function(path, depth, trailing){
  ...
  HappnClient.prototype.__onVariableDepth = Promisify(function(path, parameters, handler, callback) {

    var _this = this;

    if (!parameters.depth) parameters.depth = 5;

    var variableDepthPermutationPaths = _this.__getVariableDepthPermutationPaths(path, parameters.depth);

    var basePath = variableDepthPermutationPaths[0]; //the primary path we use to get a base handle

    var allInitialValues;

    _this.on(basePath, parameters, handler, function(e, handle){

      if (e) return callback(e);

      var baseHandle = handle;

      _this.state.variableDepthSubscriptions[baseHandle] = [];

      variableDepthPermutationPaths.shift();//remove the first base subscription

      _this.utils.async(variableDepthPermutationPaths, function(variableDepthPermutationPath, index, next) {

        _this.on(variableDepthPermutationPath, parameters, handler, function(e, handle, initialValues){

          if (e) {
            //TODO: rollback, may be painful
            return next(e);
          }
          _this.state.variableDepthSubscriptions[baseHandle].push(handle);
          if (initialValues) {
            if (!allInitialValues) allInitialValues = [];
            initialValues.forEach(function(initialValue){
        ...
        if (e) return callback(e);
        callback(null, baseHandle, allInitialValues);
      });
    });
  });
  ...
  HappnClient.prototype.on = Promisify(function(path, parameters, handler, callback) {
    ...
    //variable depth trailing call
    if (path.substring(path.length - 3) == '/**') return _this.__onVariableDepth(path, parameters, handler, callback);
    ...
    if (_this.state.refCount[listener.eventKey] == 1 || listener.initialCallback || listener.initialEmit)
      return _this._remoteOn(path, parameters, function(e, response) {
        ...
        if (e || (response && response.status == 'error')) {
  ...
  HappnClient.prototype.__offVariableDepthListeners = function(handle, callback){

    var _this = this;

    _this.utils.async(_this.state.variableDepthSubscriptions[handle].slice(), function(variableDepthHandle, index, next) {

      _this._offListener(variableDepthHandle, function(e){

        if (e) return next(e);
        //remove the item from the array
        _this.state.variableDepthSubscriptions[handle].splice(1, _this.state.variableDepthSubscriptions[handle].indexOf(variableDepthHandle));
        return next();
      });
    }, function(e){
      if (e) return callback(e);
      delete _this.state.variableDepthSubscriptions[handle];
      callback();
    });
  };
  ...
  HappnClient.prototype._offListener = function(handle, callback) {
    ...
    Object.keys(_this.state.events).every(function(channel) {
      ...
      return listeners.every(function(listener) {
        ...
        if (_this.state.variableDepthSubscriptions[listener.id] != null){

          return _this.__offVariableDepthListeners(listener.id, function(e){
  ...
  HappnClient.prototype.disconnect = Promisify(function(options, callback) {
    ...
    try {
      ...
      this.state.variableDepthSubscriptions = {};
      this.status = STATUS.DISCONNECTED;
