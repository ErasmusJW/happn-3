...
SessionService.prototype.stats = function () {
  return {
    ...
    messagesInPlay:this.messagesInPlay
  }
};
...
SessionService.prototype.initializeCaches = function(callback){
  ...
  if (!this.config.queuedDisconnectsCache) this.config.queuedDisconnectsCache = {type:'static'};
...
SessionService.prototype.doDisconnect = function(message, callback){
  ...
  try{
    ...
    if (message.request.action === 'disconnect'){
      ...
      _this.__queuedDisconnects.set(message.session.id, 1, {ttl:_this.config.queuedDisconnectsTTL}, function(e){
...
SessionService.prototype.initialize = function (config, callback) {
  ...
  try {
    ...
    _this.systemErrorHandler = function(e){

      _this.happn.services.error.handleSystem(e);
      _this.messagesInPlay--;
    };
...
SessionService.prototype.handleMessage = Promise.promisify(function (message, client) {
  ...
  if (message.data) {
    steps.push({step:'Set', timestamp:message.data?message.data.step_timestamp:hrTime()});
  }

  steps.push({step:'Session', timestamp:hrTime()});

  this.messagesInPlay ++;

  _this.happn.services.queue.pushInbound({raw:message, session:_this.__sessions[client.sessionId].data, id:_this.__currentMessageId, steps:steps})

    .then(function(processed){

      processed.response.__outbound = true;

      processed.response._meta.steps = processed.steps;
    ...
    .catch(_this.systemErrorHandler);
