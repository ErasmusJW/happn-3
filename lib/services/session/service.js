...
function __revokeSession(session, reason, callback) {
  ...
  this.happn.services.security.revokeSession(session, reason, (e) => {
    callback(e);
  });
}
...
function __updateSession(sessionId, updated) {
  ...
  for (var propertyName in updated) this.__sessions[sessionId].data[propertyName] = updated[propertyName];
...
function each(eachHandler, callback) {
  ...
  async.each(sessionKeys, (sessionKey, sessionKeyCallback) => {
...
function disconnectAllClients(callback) {
  ...
  this.each((sessionData, sessionDataCallback) => {
    this.disconnectSession(sessionData.id, sessionDataCallback, {reason:'reconnect-false'});
  }, callback);
}
...
function initialize(config, callback) {
  ...
  try {
    ...
    if (process.env.UPDATE_BROWSER_PRIMUS) this.primus.save(this.script);
...
function handleMessage(message, client) {
  ...
  this.happn.services.queue.pushInbound({
      raw: message,
    ...
    .catch((e) => {
      this.happn.services.error.handleSystem(e, 'SessionService.handleMessage', constants.ERROR_SEVERITY.MEDIUM);
    });
}
...
function securityDirectoryChanged(whatHappnd, changedData, effectedSessions) {
  ...
  return new Promise((resolve, reject) => {
    ...
    async.each(effectedSessions, (effectedSession, sessionCB) => {
      try{
