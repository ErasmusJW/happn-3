...
SystemService.prototype._ensureSystemName = function (config, callback) {
  ...
  this.dataService.get('/_SYSTEM/_NETWORK/_SETTINGS/NAME', {}, (e, response) => {

    if (e) return callback(e);

    if (!response) {

      if (!config.name) config.name = this.uniqueName();

      return this.dataService.upsert('/_SYSTEM/_NETWORK/_SETTINGS/NAME', config.name, {}, (e, result) => {

        if (e) return callback(e);
        this.name = result.data.value;
        this.happn.name = result.data.value;
        callback();
      ...
      if (config.name && response.data.value != config.name) {
        ...
        this.happn.name = response.data.value;
      }
    }
...
SystemService.prototype.processMessage = Promise.promisify(function (message, callback) {
  try {
    ...
    if (message.request.action === 'describe') message.response = {
