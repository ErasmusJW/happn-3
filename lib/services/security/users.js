...
function getUser(userName, options, callback) {
  ...
  this.dataService.get(userPath, function(e, foundUser) {

    if (e) return callback(e);

    if (!foundUser) return callback(null, null);

    var user = _this.securityService.serialize('user', foundUser, options);
    var password = foundUser.data.password;
    user.groups = {};

    _this.dataService.get(userPath + '/_USER_GROUP/*', {
      sort: {
        'path': 1
      }
    }, function (e, userGroups) {
...
function __getUserNamesFromGroupLinks(userGroupLinks){
  ...
  return userGroupLinks.map(function(link){
...
function listUserNamesByGroup(groupName){
  ...
  return new Promise(function(resolve, reject){
    if (typeof groupName != "string") return reject(new Error('validation error: groupName must be specified'));
    _this.dataService.get('/_SYSTEM/_SECURITY/_USER/*/_USER_GROUP/' + groupName, {path_only:true})
      .then(function(userGroupLinks){
        resolve(_this.__getUserNamesFromGroupLinks(userGroupLinks));
      })
      .catch(reject);
  });
}
...
function listUsersByGroup(groupName, options, callback) {
  ...
  if (typeof groupName != "string") return callback(new Error('validation error: groupName must be specified'));
  ...
  _this.listUserNamesByGroup(groupName)
    .then(function(usernames){
      if (usernames.length == 0) return callback(null, []);
      userSearchCriteria.$and.push({'username':{$in:usernames}});
      _this.dataService.get('/_SYSTEM/_SECURITY/_USER/*', {criteria: userSearchCriteria}, function (e, results) {
        if (e) return callback(e);
