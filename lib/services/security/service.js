...
function processLogin(message) {
  ...
  return new Promise((resolve, reject) => {
    ...
    this.login(credentials, sessionId, message.request, (e, session) => {
      ...
      if (!attachedSession) return reject(new Error('session with id ' + sessionId + ' dropped while logging in'));
...
function resetSessionPermissions(whatHappnd, changedData) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (['link-group', 'unlink-group', 'delete-user', 'delete-group', 'upsert-group', 'permission-removed', 'permission-upserted', 'upsert-user'].indexOf(whatHappnd) == -1) return resolve();
    ...
    this.sessionService.each((sessionData, sessionCallback) => {
      ...
      if (whatHappnd == 'delete-user') {
        ...
        if (sessionData.user.username == userName) {
          ...
          // not necessary this session is over now
        }
      }
...
function login(credentials, sessionId, request, callback) {
  ...
  if (credentials.username == '_ADMIN' && this.config.disableDefaultAdminNetworkConnections === true && request.data.info._local == false)
  ...
  if (credentials.token) {
    ...
    try {
      ...
      if (previousSession.username == '_ADMIN' && this.config.disableDefaultAdminNetworkConnections === true && request.data.info._local == false)
        return callback(this.happn.services.error.AccessDeniedError('use of _ADMIN credentials over the network is disabled'));
