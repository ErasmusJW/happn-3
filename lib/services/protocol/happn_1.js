...
ProtocolHappn.prototype.transformIn = Promise.promisify(function (message, callback) {
    ...
    if (message.raw.encrypted || (message.raw.data && message.raw.data.encrypted)) {
      ...
      if (message.raw.action === 'login') {
        ...
        return callback(null, this.__transformAndValidate(message));
      }
...
ProtocolHappn.prototype.__transformAndValidate = function (message) {

  if (message.request){

    if (message.request.action == 'off' && message.request.options && message.request.options.listenerId != null)
    ...
    if (['on','set','remove'].indexOf(message.request.action) > -1)
...
ProtocolHappn.prototype.fail = function (message, callback) {
  ...
  message.response = _this.__createResponse(message.error, message.request || message.raw, message.response, message.opts);

  if (message.request.action != 'login' && message.session.isEncrypted) { //there is no session secret if the login failed, so login failure responses cannot be encrypted
    message.response = {
