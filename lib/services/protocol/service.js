...
ProtocolService.prototype.__getInboundStack = function(transformed){
  ...
  if (!this.__stackCache[stackKey]){
    ...
    cachedStack.push(function (message, callback){
      message.steps.push({step:'stack begin', timestamp:hrTime()});
      callback(null, message);
    });
    ...
    if (this.config.secure){
      ...
      cachedStack.push(function (message, callback){
        message.steps.push({step:'authorized', timestamp:hrTime()});
        callback(null, message);
      });
    }
    ...
    if (transformed.request.action === 'set'){
      ...
      if (transformed.request.options && transformed.request.options.noStore) {
        cachedStack.push(this.happn.services.layer.data.doNoStore);

        cachedStack.push(function (message, callback){
          message.steps.push({step:'no-stored', timestamp:hrTime()});
          callback(null, message);
        });
      }
      else {
        ...
        cachedStack.push(function (message, callback){
          message.steps.push({step:'stored', timestamp:hrTime()});
          callback(null, message);
        });
      }
      ...
      else cachedStack.push(this.happn.services.layer.pubsub);

      cachedStack.push(function (message, callback){
        message.steps.push({step:'published', timestamp:hrTime()});
        callback(null, message);
      });
    }
      ...
      cachedStack.push(this.happn.services.layer.session.doDisconnect);
    }
...
ProtocolService.prototype.__respondMessageInLayers = function(protocol, message, respond, e){
  ...
  try{
    ...
    async.waterfall(responseLayers, function(e, message){
      ...
      if (e) return _this.happn.services.error.handleSystem(e);
...
ProtocolService.prototype.processMessageIn = function(message, callback){
  ...
  message.steps.push({step:'popped', timestamp:hrTime()});
  ...
  if (message.raw.steps) message.raw.steps.push({step:'popped', timestamp:hrTime()});

  if (message.request && message.request.steps) message.request.steps.push({step:'popped', timestamp:hrTime()});

  protocol.transformIn(message)

    .then(function (transformed) {

      transformed.steps.push({step:'transformed', timestamp:hrTime()});
