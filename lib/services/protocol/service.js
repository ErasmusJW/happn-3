...
ProtocolService.prototype.__getInboundStack = function(transformed){
  ...
  if (!transformed.request.options) transformed.request.options = {};

  var stackKey = transformed.request.action + JSON.stringify(transformed.request.options);
  ...
  if (!this.__stackCache[stackKey]){
    ...
    if (transformed.request.action === 'set'){
      ...
      if (transformed.request.options.noStore) cachedStack.push(this.happn.services.layer.data.doNoStore);
      ...
      if (!transformed.request.options.noPublish) cachedStack.push(this.happn.services.layer.pubsub.doPublish);
    }
      ...
      cachedStack.push(this.happn.services.layer.pubsub.doSubscribe);
    }

    else if (transformed.request.action == 'off'){

      cachedStack.push(this.happn.services.layer.pubsub.doUnsubscribe);
    }
