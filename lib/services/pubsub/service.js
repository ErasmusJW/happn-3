...
PubSubService.prototype.stats = function (opts) {
  return {
    listeners_SET: Object.keys(this.__listeners_SET).length,
    listeners_REMOVE: Object.keys(this.__listeners_REMOVE).length,
    listeners_ALL: Object.keys(this.__listeners_ALL).length,
    listeners_ONALL: Object.keys(this.__listeners_ONALL).length,
    listeners_wildcard_ALL: Object.keys(this.__listeners_wildcard_ALL).length,
    listeners_wildcard_SET: Object.keys(this.__listeners_wildcard_SET).length,
    listeners_wildcard_REMOVE: Object.keys(this.__listeners_wildcard_REMOVE).length
  }
};

PubSubService.prototype.doPublish = function(message, callback){
  try{

    this.publish(message.request, message.response);
...
PubSubService.prototype.doSubscribe = function(message, callback){
...
PubSubService.prototype.doUnsubscribe = function(message, callback){
...
PubSubService.prototype.processMessage = Promise.promisify(function(message, callback){
  try{
    ...
    if (message.request.action === 'on') {
      ...
      if (message.request.options && message.request.options.noPublish) _this.publish(message.request, message.response);
    }
...
PubSubService.prototype.__emitInitialValues = function (eventId, channel, sessionId, initialItems, protocol, callback) {
  ...
  initialItems.forEach(function(item){
    ...
    _this.__emitPublication(item, channel, {clone:true, protocol:protocol}, sessionId);
  ...
  if (message.request.options.initialCallback || message.request.options.initialEmit) {
    ...
    _this.dataService.get(dataPath, {sort: {'modified': 1}}, function (e, initialItems) {
      ...
      if (message.request.options.initialCallback) {
        ...
        doCallback();
        console.log('emitting initial:::',message.request.eventId, channel, message.session.id, initialItems, message.protocol);
...
PubSubService.prototype.__emitPublication = function (publication, channel, opts, sessionId){
...
PubSubService.prototype.emitToAudience = function (publication, channel, opts) {
  ...
  var serialized = JSON.stringify(publication);

  if (!audienceGroup || !audienceGroup[channel]) return;

  var audienceSessions = Object.keys(audienceGroup[channel]);

  if (audienceSessions.length == 0) return;

  audienceSessions.forEach(function(sessionId){

    _this.__emitPublication(JSON.parse(serialized), channel, {clone:false, noCluster:opts.noCluster}, sessionId);
  });

};

PubSubService.prototype.buildAudience = function(request){
...
PubSubService.prototype.publish = function (request, payload) {
  ...
  _this.emitToAudience(payload, messageChannel, opts);

  opts.targetAction = 'ALL';

  _this.emitToAudience(payload, '/ALL@' + request.path, opts);

  opts.hasWildcard = true;

  for (var allPath in _this.__listeners_wildcard_ALL){
    if (_this.happn.services.utils.wildcardMatch(allPath.replace('/ALL@/', '/*@/'), messageChannel)) _this.emitToAudience(payload, allPath, opts);
  }

  opts.targetAction = action;

  for (var actionPath in _this['__listeners_wildcard_' + action]){
    if (_this.happn.services.utils.wildcardMatch(actionPath, messageChannel)) _this.emitToAudience(payload, actionPath, opts);
  }

  _this.emitToAudience(payload, '/ALL@*', opts);
