...
DataService.prototype.processGet = Promise.promisify(processGet);
DataService.prototype.processRemove = Promise.promisify(processRemove);
...
function initialize(config, callback) {
  ...
  if (this.config.datastores.length == 0) { //insert the default nedb data store
    ...
    if (this.config.compactInterval) defaultDatastoreConfig.settings.compactInterval = this.config.compactInterval;
...
function __updateDatabaseVersions(){
  ...
  return new Promise((resolve, reject) => {
    ...
    versionUpdater.analyzeDB((e, analysis) => {
      ...
      if (!this.config.autoUpdateDBVersion) return reject(new Error('current database version ' + analysis.currentDBVersion + ' does not match ' + analysis.moduleDBVersion));

      versionUpdater.updateDB(analysis, (logs) => {

        this.log.warn('database upgrade from ' + analysis.currentDBVersion + ' to ' + analysis.moduleDBVersion);
...
function get(path, parameters, callback) {
  ...
  provider.find(path, options, (e, items) => {
...
function processGet(message, callback) {
  ...
  return this.get(message.request.path, message.request.options, (e, response) => {
    ...
    if (e) return this.errorService.handleSystem(new Error('processGet failed', e), 'DataService', constants.ERROR_SEVERITY.HIGH, (e) => {
      callback(e, message);
    });
...
function processRemove(message, callback) {
  ...
  return this.remove(message.request.path, message.request.options, (e, response) => {
    ...
    if (e) return this.errorService.handleSystem(new Error('processRemove failed', e), 'DataService', constants.ERROR_SEVERITY.HIGH, (e) => {
      callback(e, message);
    });
...
function processStore(message) {

  if (!message.request.options) message.request.options = {};
  if (message.request.options.noStore) return this.processNoStore(message);

  return new Promise((resolve, reject) => {

    this.upsert(message.request.path, message.request.data, message.request.options, (e, response) => {

      if (e) return this.errorService.handleSystem(new Error('processStore failed', e), 'DataService', constants.ERROR_SEVERITY.HIGH, (e) => {
        reject(e);
      });
...
function processSecureStore(message) {
...
function processNoStore(message) {
...
function getOneByPath(path, fields, callback) {
  ...
  this.db(path).findOne({
    ...
    if (e) return this.errorService.handleSystem(e, 'DataService', constants.ERROR_SEVERITY.MEDIUM, callback);
...
DataService.prototype.__upsertInternal = function (path, setData, options, dataWasMerged, callback) {
  ...
  if (options.increment != null) {
    ...
    if (typeof provider.increment != 'function') return callback(new Error('db provider does not have an increment function'));

    if (!setData.data || typeof setData.data.value != 'string') return callback(new Error('invalid increment counter field name, must be a string'));

    if (isNaN(options.increment)) return callback(new Error('increment option value must be a number'));
...
function startCompacting(dataStoreKey, interval, callback, compactionHandler) {
  ...
  try {
    ...
    this.__iterateDataStores(dataStoreKey, (key, dataStore, next) => {
    ...
    this.errorService.handleSystem(new Error('failure doing db compaction', err), 'DataService', constants.ERROR_SEVERITY.HIGH, callback);
  }
}
...
function compact(dataStoreKey, //what dataStore - if undefined we will compact all data stores
  ...
  if (dataStoreKey == null) {
    ...
    if (interval) this.__compacting[dataStoreKey].provider.startCompacting(interval, callback, compactionHandler);

    else {

      this.__compacting[dataStoreKey].provider.compact((e) => {

        if (e) this.errorService.handleSystem(new Error('data provider compaction failed', e), 'DataService.compaction', constants.ERROR_SEVERITY.MEDIUM);
...
function __initializeProviders() {
  ...
  return new Promise((resolve, reject) => {
    ...
    async.eachSeries(this.config.datastores, (datastoreConfig, datastoreCallback) => {
      ...
      try {
        ...
        dataStoreInstance.provider.initialize((e) => {
          ...
          if (dataStoreInstance.provider.transform == null) dataStoreInstance.provider.transform = this.transform;
          if (dataStoreInstance.provider.transformAll == null) dataStoreInstance.provider.transformAll = this.transformAll;
      ...
      if (Object.keys(this.dataroutes).length == 0)
        this.db = (path) => {
        ...
        this.db = (path) => {
          for (var dataStoreRoute in this.dataroutes) {
            if (this.happn.services.utils.wildcardMatch(dataStoreRoute, path)) return this.dataroutes[dataStoreRoute].provider;
          }
          return this.defaultProvider;
        };
...
function __iterateDataStores(dataStoreKey, operator, callback) {
  ...
  try {
    ...
    if (dataStoreKey) {
      ...
      if (!this.datastores) return callback(new Error('datastore with key ' + dataStoreKey + ', specified, but multiple datastores not configured'));

      if (!this.datastores[dataStoreKey]) return callback(new Error('datastore with key ' + dataStoreKey + ', does not exist'));
    ...
    if (this.datastores) {
      ...
      async.eachSeries(Object.keys(this.datastores), (key, next) => {
