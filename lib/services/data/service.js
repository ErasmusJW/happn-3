...
DataEmbeddedService.prototype.doGet = Promise.promisify(function(message, callback){
...
DataEmbeddedService.prototype.doRemove = Promise.promisify(function(message, callback){
...
DataEmbeddedService.prototype.doStore = Promise.promisify(function(message, callback){
...
DataEmbeddedService.prototype.doSecureStore = Promise.promisify(function(message, callback){
...
DataEmbeddedService.prototype.doNoStore = Promise.promisify(function(message, callback){
...
DataEmbeddedService.prototype.randomId = function(){
...
DataEmbeddedService.prototype.get = function (path, parameters, callback) {
  ...
  try {
    ...
    var dbCriteria = _this.getPathCriteria(path);

    if (parameters.criteria) dbCriteria.$and.push(_this.parseFields(parameters.criteria));
    ...
    cursor.exec(function (e, items) {
      ...
      if (parameters.options.path_only) {
        ...
        return callback(e, {
          paths: _this.transformAll(items)
        });
      }
      ...
      callback(null, _this.transformAll(items));
...
DataEmbeddedService.prototype.formatSetData = function (path, data, options) {
...
DataEmbeddedService.prototype.upsert = function (path, data, options, callback) {
  ...
  var setData = _this.formatSetData(path, data, options);
  ...
  if (options.merge) {
    ...
    return _this.getOneByPath(path, null, function (e, previous) {
      ...
      if (!previous) {
        ...
        if (options.tag)  return callback(new Error('Attempt to tag something that doesn\'t exist in the first place'));
...
DataEmbeddedService.prototype.transformAll = function (items) {
...
DataEmbeddedService.prototype.__upsertInternal = function (path, setData, options, dataWasMerged, callback) {
  ...
  var setParameters = {$set: {'data': setData.data, '_id': setData._meta.path, 'path': setData._meta.path, 'modifiedBy':setData._meta.modifiedBy}};
  ...
  _this.db(path).update({'_id': path}, setParameters, {upsert: true}, function (err, response, created, upsert, meta) {
    ...
    if (dataWasMerged) {
      ...
      if (options.tag) return this.insertTag(setData, options.tag, path, callback);//_this.saveTag(path, options.tag, setData, callback);// we have a prefetched object, and we want to tag it
...
DataEmbeddedService.prototype.getPathCriteria = function(path){
...
DataEmbeddedService.prototype.remove = function (path, options, callback) {
  ...
  this.db(path).remove(this.getPathCriteria(path),
